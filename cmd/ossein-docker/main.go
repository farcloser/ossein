...
// does not implement is worse than no shim. And where docker and ossein
// disagree, docker wins: a run or build that sizes nothing gets the whole
// host, as it would from a daemon, not ossein's own defaults.
package main
...
func main() {
	...
	kctx := kong.Parse(&root,
		...
		kong.UsageOnError(),
		kong.Vars{"log_levels": cli.LogLevels},
	)

	logger := cli.NewLogger(root.LogLevel)

	if err := kctx.Run(logger, root.LogLevel); err != nil {
