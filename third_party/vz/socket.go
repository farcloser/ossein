...
func (v *VirtioSocketDevice) Listen(port uint32) (*VirtioSocketListener, error) {
	...
	// OSSEIN FORK: the listener is built before the handle so the delivery
	// closure can see its lifecycle state (closed, delivering). See Close.
	listener := &VirtioSocketListener{
		vsockDevice: v,
		port:        port,
		// Backlog of one. It is not a drop threshold: a delivery that finds
		// the buffer full parks in the select below until Accept drains it,
		// which is the backpressure a listener is supposed to apply.
		acceptch: make(chan connResults, 1),
		closed:   make(chan struct{}),
	}

	handle := cgo.NewHandle(func(conn *VirtioSocketConnection, err error) {
		// OSSEIN FORK: runs on the VM queue (shouldAcceptNewConnectionHandler
		// calls this synchronously), so it must not block — hence handing the
		// send to a goroutine. The Add happens HERE, on the queue, because
		// Close's removeSocketListenerForPort is dispatch_sync'd onto that
		// same serial queue: once it returns, every delivery that will ever
		// exist has already been counted, so Close's Wait cannot miss one.
		listener.delivering.Add(1)

		go func() {
			defer listener.delivering.Done()

			select {
			case listener.acceptch <- connResults{conn, err}:
			case <-listener.closed:
				// Nobody will ever Accept this. The connection owns a live
				// file descriptor, so dropping it silently would leak one per
				// connection that raced Close.
				if conn != nil {
					_ = conn.Close()
				}
			}
	...
	listener.pointer = objc.NewPointer(ptr)
...
type VirtioSocketListener struct {
	*pointer
	...
	closeOnce   sync.Once
	// OSSEIN FORK: closed is closed by Close and never sent on, so every
	// Accept — pending or later — is released exactly once and no send can
	// ever land on a closed channel. delivering counts the in-flight
	// deliveries Close must join before it may reclaim anything.
	closed     chan struct{}
	delivering sync.WaitGroup
}

// ErrListenerClosed is what Accept returns once the listener has been closed.
//
// OSSEIN FORK: upstream returned a fresh unexported error here, which callers
// could only match on its text. net/http's Serve loop and ossein's own accept
// loops both need to tell "we shut this down" apart from a real failure.
var ErrListenerClosed = errors.New("accept failed: listener has been closed")
...
//
// It returns ErrListenerClosed once Close has run — including for calls made
// after Close, which upstream left blocked forever on a channel that had
// already handed its single sentinel to whoever was waiting first.
...
// Close stops listening on the virtio socket. It is safe to call concurrently
// with Accept and with a guest connecting, and it never returns while a
// delivery is still in flight.
func (v *VirtioSocketListener) Close() error {
	v.closeOnce.Do(func() {
		// Deregister FIRST. This is dispatch_sync'd onto the VM queue, which
		// is the same serial queue the accept delegate runs on, so once it
		// returns the framework cannot hand us another connection and every
		// delivery that will ever exist is already counted in delivering.
		...
		// OSSEIN FORK: upstream pushed a sentinel onto acceptch and closed it.
		// Both were bugs. The push blocks forever when a connection is already
		// buffered (Close is called from teardown paths that then hang), and
		// closing the channel turns any delivery still racing us into a
		// "send on closed channel" panic in a goroutine no recover covers.
		// Closing a dedicated signal channel instead is unbounded, idempotent
		// and unblocks every waiter rather than just one.
		close(v.closed)

		// Every delivery has now either landed in acceptch or closed its own
		// connection, so the drain below sees a stable channel.
		v.delivering.Wait()

		// A connection the framework delivered but nobody accepted still owns
		// a file descriptor.
		for {
			select {
			case result := <-v.acceptch:
...
func shouldAcceptNewConnectionHandler(cgoHandleUintptr C.uintptr_t, connPtr, devicePtr unsafe.Pointer) C.bool {
	...
	// OSSEIN FORK: called synchronously, not `go handler(...)`. The handler
	// itself does the only thing that must happen on this queue — registering
	// the delivery so Close cannot race past it — and hands the blocking send
	// to a goroutine of its own. Spawning here instead left the registration
	// unordered against Close, which is what let a delivery outlive it.
	handler(conn, err)
...
// CloseWrite shuts down the writing half of the connection, leaving the
// reading half open: the peer sees EOF while it can still send.
//
// OSSEIN FORK: upstream exposes neither this nor the underlying conn, so a
// caller relaying between a vsock hop and something else had no way to
// propagate a half-close and had to tear the whole connection down — which
// cuts off a peer that is still replying, and breaks every EOF-framed
// protocol through such a relay (see pkg/container's bidiPipe).
//
// Nothing exotic is needed to support it. The framework hands over a socket
// file descriptor and newVirtioSocketConnection already wraps it with
// net.FileConn, whose concrete type implements CloseWrite; it was only ever
// hidden behind an unexported field. The interface assertion keeps that
// honest: if a future macOS hands back a descriptor family net.FileConn maps
// to something without a write shutdown, this reports it instead of silently
// half-working.
func (v *VirtioSocketConnection) CloseWrite() error {
	closer, ok := v.rawConn.(interface{ CloseWrite() error })
	if !ok {
		return fmt.Errorf("vsock connection: %T does not support a write shutdown", v.rawConn)
	}
