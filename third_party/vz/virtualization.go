...
func NewVirtualMachine(config *VirtualMachineConfiguration) (*VirtualMachine, error) {
	...
	// OSSEIN FORK: the disconnect stream is gone (see FORK.md), but the C
	// constructor's signature and the delegate that calls back into
	// emitAttachmentWasDisconnected are untouched — pruning the Objective-C
	// side is what a previous attempt got wrong. Pass a zero handle: the
	// exported callbacks below never dereference it.
	const disconnectedHandle = 0
...
func changeStateOnObserver(newStateRaw C.int, cgoHandleUintptr C.uintptr_t) {
	...
	v.mu.Lock()
	v.state = VirtualMachineState(newStateRaw)
	v.mu.Unlock()
}

// State represents execution state of the virtual machine.
//
// OSSEIN FORK: this is the ONLY consumer of the KVO observer now.
// StateChangedNotify, which published the same transitions onto an unbounded
// channel, was removed — see FORK.md. The cached field this reads is written
// by changeStateOnObserver, so callers wanting to wait for a state should poll
// this (level-triggered, immune to a missed edge) rather than expect events.
...
// OSSEIN FORK: both callbacks below are discards.
//
// The Objective-C delegate still invokes them (virtualization_11.m:82,97) and
// the C signatures are unchanged, so nothing on that side had to be touched.
// What went away is the Go plumbing they fed: an unbounded channel pair whose
// only reader was NetworkDeviceAttachmentWasDisconnected, which ossein never
// called — so every network hiccup queued an entry nobody would ever take.
//
// To restore: take NetworkDeviceAttachmentWasDisconnected, watchDisconnected,
// the disconnected/DisconnectedError types and these two bodies back from
// upstream. The cgoHandle argument is passed as 0 by NewVirtualMachine and
// must become a real handle again first.
...
func emitAttachmentWasDisconnected(index C.int, errPtr unsafe.Pointer, cgoHandleUintptr C.uintptr_t) {
	// newNSError owns the NSError bridging; call it so the error is consumed
	// the same way it always was, then drop it.
	_ = newNSError(errPtr)
}
