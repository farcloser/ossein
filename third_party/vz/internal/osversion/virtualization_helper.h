...
// for macOS 26 API
//
// OSSEIN FORK: this is a HARD requirement, not an optional capability.
// ossein's networking is built on vmnet_network_* (macOS 26), and upstream's
// behaviour when the SDK is older is to compile the calls out and raise an
// Objective-C exception AT RUNTIME — i.e. a VM that boots and then has no
// network, from a build that looked fine. Fail the build instead.
#if __MAC_OS_X_VERSION_MAX_ALLOWED >= 260000
#define INCLUDE_TARGET_OSX_26 1
#else
#error "ossein requires the macOS 26 SDK or newer (vmnet_network_* networking). Update Xcode / Command Line Tools."
#endif
